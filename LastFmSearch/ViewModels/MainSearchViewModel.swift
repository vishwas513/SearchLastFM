...
final class MainSearchViewModel {
    ...
    func search(searchTerm: String, completion: @escaping (NetworkError?) -> Void) {
        ...
        searchAlbums(searchTerm: searchTerm, apiKey: apiKey, completion: {
            error in
        ...
        searchArtists(searchTerm: searchTerm, apiKey: apiKey, completion: {
            error in
            completion(error)
            
        })
        
        searchTracks(searchTerm: searchTerm, apiKey: apiKey, completion: {
            error in
            completion(error)
        })
    ...
    func searchAlbums(searchTerm: String, apiKey: String, completion: @escaping (NetworkError?) -> Void) {
        let path = "?method=album.search&album=\(searchTerm)&api_key=\(apiKey)&format=json"
    ...
    func searchArtists(searchTerm: String, apiKey: String, completion: @escaping (NetworkError?) -> Void) {
        
        ///2.0/?method=artist.search&artist=cher&api_key=YOUR_API_KEY&format=json
        let path = "?method=artist.search&artist=\(searchTerm)&api_key=\(apiKey)&format=json"
        let request = APIRequest.get(withPath: path)
    ...
    func searchTracks(searchTerm: String, apiKey: String, completion: @escaping (NetworkError?) -> Void) {
        let path = "?method=track.search&track=\(searchTerm)&api_key=\(apiKey)&format=json"
        let request = APIRequest.get(withPath: path)
